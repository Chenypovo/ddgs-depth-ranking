...
def render(cam,g,pipe,bg):
    return raw_render(cam,OpacityExpectation(g,0.85),pipe,bg)
...
outdir = root/'failure_analysis'/('opacity085_'+model_path.name)
...
with torch.inference_mode():
    ...
    summary={'state':'complete','iteration':args.iteration,'views':len(rows),'per_view':rows,
             ...
             'evidence':'fixed-checkpoint opacity 0.85 control on development scene; no retraining, no novelty claim'}
