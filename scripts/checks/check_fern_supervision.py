"""Real fern initial-scene integration; no metric-based model selection."""
import json,sys
from pathlib import Path
from types import SimpleNamespace
import numpy as np,torch
r=Path(__file__).resolve().parents[2];sys.path.insert(0,str(r/'DDGS_fern'))
from scene import Scene,GaussianModel
from gaussian_renderer import render
from utils.rank_supervision import RankSupervisor
from utils.general_utils import safe_state
safe_state(False)
folder=r/'fern_integration';folder.mkdir(exist_ok=False)
data=r/'dataset/nerf_llff_data/fern';split=json.loads((data/'3_views/split.json').read_text());assert split['state']=='complete'
...
scene=Scene(ds,g,shuffle=False);cams=scene.getTrainCameras();assert [c.image_name for c in cams]==[Path(n).stem for n in split['train_images']];assert len(scene.getTestCameras())==3
...
sup=RankSupervisor(r/'fern_priors','ordinary',0,folder/'rank_stats.jsonl');stats=[]
for cam in cams:
...
result=dict(state='passed',rows=stats,initial_points=split['dense_points'],train_images=split['train_images'],evaluation_views=3,global_rng_unchanged=True)
(r/'fern_integration.json').write_text(json.dumps(result,indent=2));print(json.dumps(result),flush=True)
