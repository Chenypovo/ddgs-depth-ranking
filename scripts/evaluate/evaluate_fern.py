...
sys.path.insert(0,os.environ.get('DDGS_SOURCE', str(root/'DDGS_fern')))
...
dataset = SimpleNamespace(sh_degree=3, source_path=str(root/'dataset/nerf_llff_data/fern'),
...
split = json.loads((root/'dataset/nerf_llff_data/fern/3_views/split.json').read_text())
...
with torch.inference_mode():
    ...
    summary={'state':'complete','iteration':args.iteration,'views':len(rows),'per_view':rows,
             ...
             'evidence':'fixed-protocol fern scene validation; not full benchmark', 'keep_probability':args.keep_probability}
