...
sys.path.insert(0,os.environ.get('DDGS_SOURCE', str(root/'DDGS_room')))
from scene import Scene, GaussianModel
from scene.cameras import Camera
from gaussian_renderer import render as raw_render
import sys
from pathlib import Path
...
def render(cam,g,pipe,bg):
    return raw_render(cam,g if args.keep_probability==1 else OpacityExpectation(g,args.keep_probability),pipe,bg)
...
p.add_argument('--keep-probability',type=float,choices=[1.,.85],default=1.)
args = p.parse_args()
model_path = args.model.resolve()
outdir = model_path/('evaluation' if args.keep_probability==1 else 'evaluation_q085')
outdir.mkdir(exist_ok=False)
dataset = SimpleNamespace(sh_degree=3, source_path=str(root/'dataset/nerf_llff_data/room'),
...
split = json.loads((root/'dataset/nerf_llff_data/room/3_views/split.json').read_text())
...
with torch.inference_mode():
    ...
    summary={'state':'complete','iteration':args.iteration,'views':len(rows),'per_view':rows,
             ...
             'evidence':'fixed-protocol room scene validation; not full benchmark', 'keep_probability':args.keep_probability}
