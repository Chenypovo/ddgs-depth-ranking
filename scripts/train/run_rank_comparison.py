...
def main():
 ...
 root=Path(__file__).resolve().parents[2];source=root/'DDGS_rank';py=root/'.venv/bin/python'
 ...
 hashes={str(f.relative_to(root)):hashlib.sha256(f.read_bytes()).hexdigest() for f in [source/'train.py',source/'utils/rank_supervision.py',source/'gaussian_renderer/__init__.py',source/'scene/gaussian_model.py',root/'scripts/evaluate/evaluate_rank.py',root/'scripts/train/run_rank_comparison.py',*[root/f'depth_reliability/view{i}.npz' for i in range(3)]]}
  ...
  for item in plan:
   ...
   if a.stage=='compare':
    ...
    for q,folder in [(1.,'evaluation'),(.85,'evaluation_q085')]:
     ...
     run([py,root/'scripts/evaluate/evaluate_rank.py','--model',model,'--keep-probability',q],model/f'eval_{q}.log',env)
